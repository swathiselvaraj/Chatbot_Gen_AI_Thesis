...
DEFAULT_SESSION_STATE = {
    ...
    'option_mapping': {f"option{i+1}": options[i] for i in range(len(options))},
    'usage_data': {
...
def extract_referenced_option(user_input: str, options: List[str]) -> Optional[str]:
    ...
    option_num = None
    for pattern in [r'(?:option|opt|op)\s*(\d+)', r'(?:^|\b)(\d+)(?:\b|$)']:
    ...
    for i, opt in enumerate(normalized_options):
        if (opt in user_input_clean or user_input_clean in opt or 
            fuzz.partial_ratio(opt, user_input_clean) > 85):
            return options[i]
...
# --- Response Handlers ---
def handle_option_reference(question: str, options: List[str], option: str, user_input: str) -> str:
    option_num = options.index(option) + 1
...
def find_exact_match(user_input: str, question_id: str) -> Tuple[Optional[str], Optional[Dict]]:
    user_input_lower = user_input.lower().strip()
    for category in ['general_followups', 'questions', 'dashboard_followups']:
        for item in data.get(category, []):
            if item.get("text", "").lower().strip() == user_input_lower:
                if category == 'questions' and item.get("question_id") != question_id:
                    continue
                return (category, item)
    return (None, None)

def handle_exact_match(match_type: str, matched_item: Dict, user_input: str, question_text: str) -> str:
    params = {
    ...
    if match_type == 'dashboard_followups':
        params['dashboard'] = True
    else:
        params['non_dashboard'] = True
    
    return get_gpt_recommendation(**params)

def handle_embedding_based_query(user_input: str, question_id: str, question_text: str) -> str:
    user_embedding = get_embedding(user_input)
    if not user_embedding:
        return "Sorry, I couldn't process your question. Please try again."

    # Check dashboard followups
...
def validate_followup(user_input: str, question_id: str, options: List[str], question_text: str = "") -> str:
    try:
        ...
        # Handle greetings (quick response without API calls)
        if user_input.lower().rstrip('!?.,') in {"hi", "hello", "hey", "greetings"}:
            st.session_state.last_recommendation = None
            return "Hello! I can help with survey questions. What would you like to know?"

        # Check for direct option references
        referenced_option = extract_referenced_option(user_input, options or st.session_state.get('original_options', []))
        if referenced_option:
            return handle_option_reference(question_text, options, referenced_option, user_input)

        # Check for exact matches in our knowledge base
        match_type, matched_item = find_exact_match(user_input, question_id)
        if match_type:
            return handle_exact_match(match_type, matched_item, user_input, question_text)

        # Fall back to embedding-based approach
        return handle_embedding_based_query(user_input, question_id, question_text)

    except Exception as e:
        st.error(f"Error in followup validation: {str(e)}")
        return "Sorry, I encountered an error processing your question."

# --- GPT Recommendation Function (Your Original Implementation) ---
...
"""
        ...
        # (Maintain exactly as you had it)
        # This is where your specific prompts live
