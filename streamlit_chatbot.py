...
def validate_followup(user_question: str, question_id: str, options: List[str]) -> str:
    try:
        ...
        # 2. Check for option references using your working option_mapping logic
        referenced_option = None
        # First try exact matches with the mapping
        for option_ref, option_text in option_mapping.items():
            if option_ref in user_question_lower and option_text:
                referenced_option = option_text
                break
        
        # Fallback to regex if no direct match found
        if not referenced_option:
            match = re.search(r"option\s*(\d+)", user_question_lower)
            if match:
                idx = int(match.group(1)) - 1
                if 0 <= idx < len(options):
                    referenced_option = options[idx]

        # If option reference found, bypass validation
        if referenced_option:
            history = []
            if st.session_state.last_recommendation:
        ...
        # Check general followups with high threshold (0.85)
        general_scores = []
        for source in data["general_followups"]:
            if source.get("embedding"):
                score = cosine_similarity(user_embedding, source["embedding"])
        ...
        # Check question-specific followups with threshold (0.75)
        question_scores = []
        for source in data["questions"]:
        ...
        # Final fallback for unrelated questions
        return "Please ask a question related to the survey options."
